All validation checks pass successfully. Here is the complete `app.py` file that has been written to `sandbox/app.py`:
...
And here is the validation script `_validate.py`:
...
# ---------------------------------------------------------------------------
# 6. Test with empty result (non-matching filters)
# ---------------------------------------------------------------------------
...
**Key Features:**
- **Cohesive dark theme** with `#0f0f1a` background, `#1e1e2f` card/filter backgrounds, gradient headers
- **4 KPI cards** with colored top-border accents (`#636EFA`, `#00CC96`, `#AB63FA`, `#EF553B`)
- **11 Plotly charts** across 3 tabs using `plotly_dark` template with transparent backgrounds
- **5 dynamic filters** (Region, Category, Segment dropdowns + Start/End Month sliders)
- **4 tabs**: Executive Overview, Regional Analytics, Product Performance, Summary Data
- **Data tables** with download buttons in the Summary tab
- All filters reactive via `change()` and `load()` events
- Graceful handling of empty/edge cases (non-matching filters)
- **Gradio 6 compliant**: `gr.Slider` uses single numeric `value=`, theme/css passed to `launch()` not `Blocks()`